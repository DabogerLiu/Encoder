...
def get_file_names(path='images', train=1):
...
def G(in_w = (w_hei, w_wid, 1), in_c = (height, width, 3), scale=1):
    ...
    #W1 = Conv2D(32, (3,3), name='En_W_2', padding='same')(C1)
...
def R(in_m = (height, width, 3), scale = 1): #128 128 3
    ...
    M1 = wm_dec(M1, scale, 'm1_dec_')

    W_prime = Conv2D(1, kernel_size=(3, 3), padding='same', strides=1, activation='sigmoid', name='W_prime')(M1)
...
if __name__ == "__main__":
    ...
    train(epochs=20)
    if send_mail(mailto_list, "wmnn 18 on GPUstation finished", "0_whole.py"):  
